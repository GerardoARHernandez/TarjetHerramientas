...
const RedeemPurchaseModal = ({ isOpen, onClose, businessName }) => {
    ...
    const startScanner = async () => {
        try {
            ...
            // Configurar hints para mejores resultados
            const hints = new Map();
            ...
            // Inicializar el lector con los hints
            ...
            // Primero buscar cámara trasera por nombre
            if (videoDevices.length > 0) {
                // Buscar específicamente cámara trasera o environment
                ...
                // Si no encuentra, usar la última cámara (generalmente la trasera en móviles)
                if (!selectedDevice) {
                    selectedDevice = videoDevices[videoDevices.length - 1];
                }
            }
            
            console.log('Usando dispositivo:', selectedDevice?.label || 'cámara por defecto');
            ...
            await codeReader.current.decodeFromVideoDevice(
                ...
                (result, error) => {
                    if (result) {
                        // Vibrar si está disponible (opcional)
                        ...
                        // Detener el scanner después de leer
                        stopScanner();
                    ...
                    if (error) {
                        // Solo mostrar errores que no sean "no se encontró código"
                        if (!error.message.includes('NotFoundException') && 
                            !error.message.includes('Timeout') &&
                            error.message !== '') {
                            console.error('Error de escaneo:', error);
                            setScanError('Acerque el código a la cámara');
                        }
                    }
                }
            );
                        ...
                        <div className="mb-6 animate-fade-in">
                            <div className="bg-gray-900 rounded-lg overflow-hidden relative">
                                ...
                                <div className="relative">
                                    ...
                                    {scanning && (
                                        <>
                                            <div className="absolute inset-x-0 top-1/2 h-0.5 bg-red-500 shadow-lg shadow-red-500/50 animate-scanner-line"></div>
                                ...
                                <div className="p-3 bg-gray-900">
                                    <p className="text-center text-white text-sm">
                                        {scanError ? (
                                            <span className="text-yellow-400">{scanError}</span>
                                        ) : (
